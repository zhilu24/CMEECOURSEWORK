...
# Evaluate and Plot (log-scale transformed back to original scale)
def evaluate_and_plot(X, y_log, label):
    ...
    plt.title(f"{label} dataset (count pfam)")
    plt.text(0.05, 0.95, f"RMSE = {rmse:.2f}\nR² = {r2:.3f}", transform=plt.gca().transAxes,
             fontsize=12, verticalalignment='top',
             bbox=dict(boxstyle="round,pad=0.3", edgecolor="black", facecolor="white"))
    plt.tight_layout()
    plt.savefig(f"{label.lower()}_ogt_prediction_log_model.png")
    plt.close()
...
plt.tight_layout()
plt.savefig("top20_feature_importance_log_count_model.png")
plt.close()

# Save model
joblib.dump(best_model, "best_rf_log_count_model.pkl")
print("✅ Model saved as best_rf_log_count_model.pkl")